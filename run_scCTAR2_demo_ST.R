...
#dowaload the file 'scSTAR2_0.1.1.0.tar.gz' and install the package from local path.
...
Integrated <- RunPCA(Integrated, npcs = 20, verbose = FALSE)#Run principal component analysis (PCA). Please note that the RunPCA function in the Seurat package uses random seeds by default.
Integrated <- RunTSNE(Integrated, reduction = "pca", dims = 1:20) # problem? type in pip install umap-learn
Integrated <- FindNeighbors(Integrated, reduction = "pca", dims = 1:20)#This step is used to find neighbor relationships between cells and perform cluster analysis.
...
### load gene markers
markers <- read.table("MarkerGenesByCluster_scSTAR2_H1.combined.xls",sep="\t")#Read gene marker data from a file named "MarkerGenesByCluster_scSTAR2_H1.combined.xls"
...
# including model training, visualization and significance testing based on specified parameters and data.
#What needs to be noted about the OPLSDA function is that the OPLSDA para.txt file is in the inst file of the scSTAR2R package. 
# The parameters can be changed at any time needed.
#Among them, clusters 0, 4, and 5 are classified as related to good prognosis by scSTAR2, and cluster 2 is classified as related to poor prognosis.
