...
OPLSDA = function(data1, data2, variables){
  ...
  dirOPLSpara<-paste(extdir,"/txt/OPLSDA para.txt",sep="")
  ...
  if (length(which(sign_buffer_1 < 0)) > 0 && length(which(sign_buffer_1 > 0)) == 0 && !is.null(model$P_Q2)) {
   text_data <- data.frame(x = 15, y = 0.0005, label = paste("p value=", as.character(model$P_Q2)))
    x11()
   p4 <- model_1$p4
  (p8 <-p4 +
  ...
  print(p8)
    ...
    text_data1 <- data.frame(x = 15, y = 0.0005, label = paste("p value=", as.character(model$P_Q2)))
    x11()
  p3 <- model_1$p3
  (p7<-p3 +
  ...
  print(p7)
    ...
    text_data2 <- data.frame(x = 35, y =0.0045 , label = paste("p value=", as.character(model$P_Q2)))
    x11()
  p2 <- model_1$p2
  (p6 <- p2 +
  ...
  print(p6)
    ...
    text_data3 <- data.frame(x = 15, y = 0.0005, label = paste("p value=", as.character(model$P_Q2)))
    x11()
  p5 <- model_1$p5
  (p9 <-p5 +
